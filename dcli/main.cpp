...
int main() {
    ...
    #ifdef NDEBUG
        std::string_view programName = "Hello from dcli!";
    #else 
        std::string_view programName = "Debug from dcli!";
    #endif
    ...
    auto renderer_text = Renderer([&] {
        auto c = Canvas(100, 100);
        c.DrawText(0, 0, "template <typename... Ts>");
        c.DrawText(0, 5, "static constexpr std::tuple<Ts...> tomfoo(Ts const&... ts) {...}");
        c.DrawText(mouse_x, mouse_y, std::string(programName),
                [](Pixel& p) {
