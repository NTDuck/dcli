...
fn main() {
    ...
    // Interactors
    let mut create_task_interactor = CreateTaskInteractor::new(&mut task_repository, &uuid_factory);
    let view_tasks_interactor = ViewTasksInteractor::new(task_repository);

    // Controllers
    let create_task_controller = CreateTaskController::new(&mut create_task_interactor);
