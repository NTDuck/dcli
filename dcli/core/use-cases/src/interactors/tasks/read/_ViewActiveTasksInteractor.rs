...
pub struct ViewActiveTasksInteractor<'i> {
    task_gateway: &'i dyn TaskGateway,
}

impl<'i> ViewActiveTasksInteractor<'i> {
...
impl<'i> utils::contracts::FunctionInteractor for ViewActiveTasksInteractor<'i> {
    fn apply(&self, request: Self::Request) -> Result<Self::Response, Self::Exception> {
        let tasks = self.task_gateway.show_active(request.offset, request.limit);
