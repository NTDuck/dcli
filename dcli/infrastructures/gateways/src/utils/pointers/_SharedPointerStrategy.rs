...
pub trait SharedPointerStrategy {
    type Typed<T>;
    type Untyped;   // type Untyped = Self::Typed<()>;
    ...
    /// so can't use `std::mem::transmute`.
    /// 
    ...
    fn into_untyped<T>(typed: Self::Typed<T>) -> Self::Untyped {
        return unsafe {
            std::mem::transmute_copy(&typed)
        };
    }

    // fn into_untyped<T>(typed: Self::Typed<T>) -> Self::Typed<()> {
    //     let ptr = Arc::into_raw(typed) as *const std::sync::Mutex<()>;
