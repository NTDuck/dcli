...
fn derive_for_ordinary_struct(ast: &syn::DeriveInput, fields: &syn::FieldsNamed) -> proc_macro2::TokenStream {
    ...
    let struct_where_clause_with_value_object_bounds = generate_where_clause_with_value_object_bounds_from_derive_input(ast);
...
fn extract_nested_meta_lists_from_meta_list(meta_list: &syn::MetaList, expected_attr_ident: &str) -> Vec<syn::MetaList> {
    return meta_list
