...
{
    fn from(
        _: RepositoryWorld<usize, InMemoryRepositoryWorldHandle<usize>>,
    ) -> Self {
        type World =
            RepositoryWorld<usize, InMemoryRepositoryWorldHandle<usize>>;

        Feature::<World>::named("Number Repository")
            .ignored(false)
            .background(
                Background::named("Nothingness")
                    .ignored(true)
                    .given("nothing", |_| ok())
                    .and("nothing", |_| ok())
            ...
            .rule(|ctx| {
                Rule::from(ctx)
                    ...
                    .ignored(true)
                    .background(
                        Background::<World>::named(
                            "Populate repository with big numbers",
                        )
                        .given("a repository with task `MAX`", |world| {
                            world.repository.add(usize::MAX);
                            ().ok()
                        })
                        .and("a repository with task `MAX - 1`", |world| {
                            world.repository.add(usize::MAX - 1);
                            ().ok()
                        })
                        .and(
                            "a repository with task `MAX - 2`",
                            |world| {
                                world.repository.add(usize::MAX - 2);
                                ().ok()
                            },
                        ),
                    )
                    .scenario(|ctx| {
                        Scenario::from(ctx)
                            ...
                            .then(
                                "the repository should contain all populated \
                                 numbers",
                                |world| {
                                    world
                                        .repository
                                        ...
                                        .expect(
                                            true,
                                            "expected `MAX` to be present, \
                                             found absent",
                                        )?;
                                    world
                                        .repository
                                        .contains(&(usize::MAX - 1))
                                        .expect(
                                            true,
                                            "expected `MAX - 1` to be \
                                             present, found absent",
                                        )?;
                                    world
                                        .repository
                                        .contains(&(usize::MAX - 2))
                                        .expect(
                                            true,
                                            "expected `MAX - 2` to be \
                                             present, found absent",
                                        )?;
            ...
            .scenario(|ctx| {
                Scenario::from(ctx)
                    ...
                    .then("the repository should contain 0", |world| {
                        world.repository.contains(&0).expect(
                            true,
                            "expected 0 to be present, found absent",
                        )
                    })
            })
            .scenario(|ctx| {
                Scenario::from(ctx)
                    ...
                    .and("1", |world| {
                        world.repository.add(1);
                        ().ok()
                    })
                    .when("removing 0", |world| {
                        world.repository.remove(&0);
                        ().ok()
                    })
                    .but("not removing 1", |_| ok())
                    .then("the repository should contain 1", |world| {
                        world.repository.contains(&1).expect(
                            true,
                            "expected 1 to be present, found absent",
                        )
                    })
                    .but("not 0", |world| {
                        world.repository.contains(&0).expect(
                            false,
                            "expected 0 to be absent, found present",
                        )
                    })
            })
            .into()
